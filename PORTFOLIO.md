...
### 시행착오: ConvNeXt 학습 데이터 부족 문제

ConvNeXt-Small은 EfficientNet이나 ResNet 계열보다 문서 이미지의 구조적 패턴을 더 풍부하게 학습할 수 있다는 장점이 있었지만, 그만큼 안정적인 fine-tuning을 위해 더 많은 입력 데이터가 필요했습니다. 초기 실험에서는 데이터 수가 부족해 모델이 특정 양식 패턴에 과하게 맞춰지거나, epoch별 검증 결과가 흔들리는 문제가 있었습니다. 특히 전입신고서처럼 레이아웃이 고정된 문서는 모델이 필드의 의미를 학습하기보다 특정 이미지의 위치와 노이즈를 외우는 방향으로 수렴할 위험이 있었습니다.

이를 해결하기 위한 첫 번째 시도로 동일한 데이터셋을 중복 구성해 학습 데이터의 절대 수를 늘렸습니다. 목적은 새로운 정보를 추가하기보다는 batch 구성과 학습 step 수를 확보해 ConvNeXt가 충분히 반복 학습할 수 있는 환경을 만드는 것이었습니다. 이 방식은 학습 로그를 안정화하고 loss가 급격히 흔들리는 현상을 줄이는 데는 일부 도움이 되었지만, 실제로 새로운 분포의 이미지가 늘어난 것은 아니기 때문에 일반화 성능을 근본적으로 높이는 데는 한계가 있었습니다.

이 과정에서 중복 데이터 확장은 "데이터 수 증가"에는 도움이 되지만 "데이터 다양성 증가"와는 다르다는 점을 확인했습니다. 이후에는 단순 복제보다 손글씨 데이터 추가, 스캔 품질 차이를 반영한 밝기/대비 증강, 문서 이미지 분포에 맞춘 normalization 재계산, Optuna 기반 하이퍼파라미터 탐색을 함께 적용하는 방향으로 실험을 조정했습니다. 결과적으로 이 시행착오는 ConvNeXt의 성능 개선이 모델 변경만으로 해결되는 문제가 아니라, 데이터 양과 데이터 다양성, 전처리, 학습 전략을 함께 설계해야 하는 문제라는 점을 보여준 실험이었습니다.
...
- ConvNeXt 모델이 상대적으로 많은 입력 데이터를 요구한다는 점을 확인하고, 중복 데이터셋 확장 실험을 통해 데이터 수 증가와 데이터 다양성 증가가 서로 다른 문제임을 학습했습니다.
